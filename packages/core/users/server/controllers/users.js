    ...
    module.exports = function(MeanUser) {
        return {
            ...
            getFreelancerStorefront : function (req,resMain) {
                var freelancer_id;
                    if(req.query.freelancerId != undefined){
                        freelancer_id = req.query.freelancerId;
                    }else{
                        if(req.user._id != undefined && req.user.roles.indexOf('freelancer') != -1){
                            freelancer_id = req.user._id;
                        }
                    }
    console.log(freelancer_id);

                Freelancer_landing.find({user_id : freelancer_id}, function(err,res){
                    if(err){
                        console.log(err);
                        resMain.json({success: false});
                    } else{
